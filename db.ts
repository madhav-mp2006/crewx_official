...
export const db = {
  ...
  // Employee Details (Profile Data Linked to Worker ID)
  getEmployeeDetails: async (userId: string) => {
    const { data, error } = await supabase
  ...
  saveEmployeeDetails: async (details: any) => {
  ...
  // Enrollments (Connected to workers)
