...
const mapEnrollment = (e: any): Enrollment => ({
  ...
  isPaid: e.is_paid ?? e.isPaid ?? false
});
...
export const db = {
  ...
  saveJob: async (job: Job) => {
    const dbJob = {
      ...
      enrolled_count: job.enrolledCount,
      status: job.status
    };
  ...
  updateJob: async (jobId: string, updates: Partial<Job>) => {
    ...
    if (updates.maxWorkers !== undefined) {
      dbUpdates.max_workers = updates.maxWorkers;
      delete dbUpdates.maxWorkers;
    }
    if (updates.enrolledCount !== undefined) {
      dbUpdates.enrolled_count = updates.enrolledCount;
      delete dbUpdates.enrolledCount;
    }
  ...
  checkEnrollment: async (userId: string, jobId: string): Promise<boolean> => {
    const { data, error } = await supabase
      .from('enrollments')
      .select('id')
      .eq('user_id', userId)
      .eq('job_id', jobId)
      .maybeSingle(); // Better than .single() as it doesn't error if 0 rows found
    
    if (error) throw error;
    return !!data;
  },
  getStaffByJob: async (jobId: string): Promise<User[]> => {
    const { data, error } = await supabase
      .from('enrollments')
      .select(`
        workers (
    ...
    if (error) throw error;
    // Extract worker objects from the join result
    return (data || []).map((item: any) => mapUser(item.workers));
  },
  ...
  markEnrollmentAsPaid: async (userId: string, jobId: string) => {
    const { error } = await supabase
      .from('enrollments')
      .update({ is_paid: true })
      .eq('user_id', userId)
