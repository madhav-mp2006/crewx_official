...
export const db = {
  ...
  saveEnrollment: async (enrollment: Enrollment) => {
    ...
    const { error } = await supabase.from('enrollments').insert(dbEnrollment);
    if (error) throw error;
  },
  ...
  saveWithdrawal: async (withdrawal: WithdrawalRequest) => {
    ...
    const { error } = await supabase.from('withdrawals').insert(dbWithdrawal);
    if (error) throw error;
  },
  ...
  saveNotification: async (notification: Partial<AppNotification>) => {
  ...
  cleanupOldNotifications: async () => {
    ...
    if (error) console.error("Auto-delete notifications failed", error);
  },
