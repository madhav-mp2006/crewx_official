...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      ...
      // Safety timeout to prevent infinite loading - reduced to 3s as requested
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (session?.user) {
        // Only sync if we don't have a user or it's a different user
  ...
  const syncUserProfile = async (supabaseUserOrEmail: any) => {
    if (isSyncingRef.current) return; // Prevent concurrent syncs using ref for immediate lock

    console.log("Syncing user profile...");
    isSyncingRef.current = true;
    setIsSyncing(true);

    // Safety timeout for sync process
    const syncTimeout = setTimeout(() => {
      if (isSyncingRef.current) {
        console.warn("Profile sync timed out");
        setIsSyncing(false);
        isSyncingRef.current = false;
        setIsLoading(false);
      }
    ...
    try {
      ...
      if (verifiedProfile) {
        ...
        userRef.current = verifiedProfile;
  ...
  const handleEnrollAttempt = (job: Job) => {
    ...
    if (!user.qrCode) {
      setActionConfirm({
        ...
        message: "Please upload your UPI QR code image before enrollment.",
        isWarning: true,
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        img.onload = async () => {
          ...
          // Max dimension 768px for better QR recognition
          const MAX_DIM = 768;
          if (width > height) {
            if (width > MAX_DIM) {
          ...
          if (!ctx) {
            setQrWarningMessage("Failed to process the uploaded image.");
          ...
          try {
            ...
            if (code) {
              ...
              setQrWarningMessage("No valid QR code detected. Please upload a clear image of your UPI QR code.");
            }
          } catch (err: any) {
            console.error("QR Decoding Error:", err);
            setQrWarningMessage("Failed to decode image. Please try another clear QR code image.");
          } finally {
            setIsProcessingQR(false);
            setQrFileToProcess(null);
          }
  ...
  const handleToggleJobStatus = async (job: Job) => {
    ...
    setActionConfirm({
      ...
      onConfirm: async () => {
        setIsSyncing(true);
        setSyncMessage("Updating event status...");
        try {
  ...
  const handleFinishJob = async (job: Job) => {
    setActionConfirm({
      title: "Finish & Pay All?",
      message: `Mark '${job.title}' as completed? This will automatically add ${job.pay} Rs to the wallet of all enrolled workers who haven't been paid yet.`,
      onConfirm: async () => {
        setIsSyncing(true);
        setSyncMessage("Processing payouts for all staff...");
        try {
          // 1. Update job status
          await db.updateJob(job.id, { status: JobStatus.COMPLETED });

          // 2. Get all unpaid enrollments for this job
          const unpaidEnrollments = enrollments.filter(e => e.jobId === job.id && !e.isPaid);

          if (unpaidEnrollments.length > 0) {
            console.log(`Processing payments for ${unpaidEnrollments.length} workers...`);

            // 3. Process each payment
            for (const enroll of unpaidEnrollments) {
              const worker = workers.find(w => w.id === enroll.userId);
              if (worker) {
                const newBalance = (worker.balance || 0) + job.pay;
                await db.updateUser(worker.id, { balance: newBalance });
                await db.markEnrollmentAsPaid(worker.id, job.id);
              }
            }
          }

          await fetchData();
          // Refresh workers list to show updated balances
          const updatedWorkers = await db.getUsers();
          setWorkers(updatedWorkers);

          setActionConfirm(null);
        } catch (err) {
          console.error("Error finishing job and paying workers:", err);
          alert("Failed to complete job and process payments. Please try again.");
        } finally {
          setIsSyncing(false);
        }
      }
  ...
  const handleUpdateJobDetails = async (e: React.FormEvent) => {
    ...
    setActionConfirm({
      ...
      onConfirm: async () => {
        setIsSyncing(true);
        setSyncMessage("Updating event details...");
        try {
  ...
  const handleAddWorkerPay = async (worker: User, amount: number, jobId: string) => {
    setActionConfirm({
      ...
      onConfirm: async () => {
        setIsSyncing(true);
        setSyncMessage(`Adding ${amount} Rs to wallet...`);
        try {
  ...
  const confirmWithdrawal = async () => {
    ...
    setIsSyncing(true);
    setSyncMessage("Submitting payout request...");
  ...
  const handleApproveWithdrawal = async (w: WithdrawalRequest) => {
    setIsSyncing(true);
    setSyncMessage("Approving payout request...");
    try {
  ...
  const handleRejectWithdrawal = async (w: WithdrawalRequest) => {
    setIsSyncing(true);
    setSyncMessage("Rejecting payout request...");
    try {
    ...
    <div className="h-screen w-screen flex flex-col lg:flex-row bg-[#020617] text-slate-100 font-inter overflow-hidden">
                  ...
                  <Card key={job.id} className="p-0 overflow-hidden border-slate-800">
                    <div className="p-6 space-y-5">
                      <div className="flex items-start justify-between">
                        ...
                        <span className={`px-3 py-1 text-[9px] font-black uppercase rounded-full border shrink-0 ${job.status === JobStatus.COMPLETED ? 'bg-slate-800 text-slate-400 border-slate-700' :
                ...
                <div className="bg-[#0f172a] border border-white/5 rounded-full p-1.5 flex items-center shadow-2xl overflow-x-auto scrollbar-hide mx-auto lg:mx-0">
                  {(['ALL', WithdrawalStatus.APPROVED, WithdrawalStatus.PENDING, WithdrawalStatus.REJECTED] as const).map(tab => (
                    <button
                      ...
                      className={`px-4 lg:px-6 py-2 lg:py-2.5 text-[9px] lg:text-[10px] font-black uppercase tracking-widest rounded-full transition-all duration-300 whitespace-nowrap ${payoutHistFilter === tab
                      ...
                      return (
                        <div key={w.id} className="px-6 lg:px-10 py-5 lg:py-7 flex flex-col md:grid md:grid-cols-4 md:items-center gap-4 md:gap-0 group hover:bg-white/[0.02] transition-all duration-300">
                          ...
                          <div className="flex items-center justify-between md:justify-center">
                            ...
                            <span className={`px-4 py-1.5 text-[9px] font-black uppercase rounded-full border transition-all duration-300 ${w.status === WithdrawalStatus.APPROVED ? 'bg-emerald-500/5 text-emerald-500 border-emerald-500/10' :
                    ...
                    <div className="p-6 space-y-5">
                      ...
                      {isEnrolled ? (
                        ...
                        <button
                          ...
                          className={`w-full py-4 rounded-[14px] text-base font-black uppercase tracking-[0.05em] transition-all shadow-xl active:scale-95 ${job.enrolledCount >= job.maxWorkers ? 'bg-slate-800 text-rose-500 cursor-not-allowed' : 'bg-[#6366f1] hover:bg-[#5558e3] text-white'}`}
                        >
          ...
          <Modal centered title={actionConfirm.title} onClose={() => setActionConfirm(null)}>
            <div className="text-center">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={actionConfirm.onConfirm} className="flex-1 py-3 bg-indigo-600 text-white font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl active:scale-95">{actionConfirm.confirmText || "YES, CONFIRM"}</button>
              </div>
            </div>
          </Modal>
        )
...

      {
        showEnrollConfirm && (
          ...
          >
            <div className="text-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 ${enrollments.some(e => e.userId === user?.id && e.jobId === selectedJobForEnroll?.id) ? 'bg-rose-500/10' : 'bg-emerald-500/10'}`}>
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowEnrollConfirm(false)} className="flex-1 py-3 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-xl text-[10px] uppercase tracking-widest active:scale-95">CANCEL</button>
                <button
                  onClick={() => handleEnrollConfirm()}
                  className="flex-1 py-3 text-white font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl active:scale-95 bg-emerald-600 hover:bg-emerald-500"
                >
                  YES, CONFIRM
                </button>
              </div>
...

      {
        qrWarningMessage && (
          <Modal centered title="Invalid Image" onClose={() => setQrWarningMessage(null)}>
            <div className="text-center">
              <div className="w-12 h-12 bg-rose-600/10 border border-rose-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
                <Icons.XMark className="w-6 h-6 text-rose-500" />
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-indigo-600 text-white font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl active:scale-95"
              >
                RE-UPLOAD QR CODE
              </button>
            </div>
