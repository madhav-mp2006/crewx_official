...
export default function App() {
  ...
  const syncUserProfile = async (supabaseUserOrEmail: any) => {
    ...
    try {
      ...
      if (!profile) {
        if (typeof supabaseUserOrEmail !== 'string') {
          // Create new profile base for Google users
          profile = {
            id: supabaseUserOrEmail.id,
            name: supabaseUserOrEmail.user_metadata.full_name || 'Worker',
            email: supabaseUserOrEmail.email!,
            role: UserRole.WORKER,
            balance: 0,
          };
        } else {
          // Create new profile for Direct Access users
          profile = {
            id: '00000000-0000-0000-0000-000000000001',
            name: 'Madhav P',
            email: supabaseUserOrEmail,
            role: UserRole.WORKER,
            balance: 0,
          };
        }
      ...
      if (verifiedProfile) {
        ...
        if (verifiedProfile.role === UserRole.ADMIN) {
          ...
          // WORKERS must finish profile if missing critical details
          const isComplete = verifiedProfile.phone && verifiedProfile.age && verifiedProfile.place;
          if (!isComplete) {
            setOnboardingData({
              name: verifiedProfile.name || '',
              phone: verifiedProfile.phone || '',
              age: verifiedProfile.age?.toString() || '',
              experience: verifiedProfile.experience?.toString() || '',
              place: verifiedProfile.place || ''
            });
            setView('onboarding');
          } else {
            setView('home');
  ...
  const handleOnboardingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updates: Partial<User> = {
        ...
        experience: parseInt(onboardingData.experience) || 0,
        place: onboardingData.place
      };
      
      await db.updateUser(user.id, updates);
      
      // Update local state and move to dashboard
  ...
  const handleEnrollConfirm = async (jobOverride?: Job) => {
    ...
    setIsSyncing(true);
    try {
      let profile = await db.getUserByEmail(user.email);
      if (!profile) {
          await db.saveUser(user);
          profile = user;
      }

      const isEnrolled = enrollments.some(e => e.userId === profile!.id && e.jobId === job.id);
      if (isEnrolled) {
        await db.deleteEnrollmentByUserAndJob(profile!.id, job.id);
      ...
      alert("Profile sync error. Please try logging out and back in to refresh your connection.");
    } finally {
      setIsSyncing(false);
      setShowEnrollConfirm(false);
  ...
  const handleAddWorkerPay = async (worker: User, amount: number) => {
    setActionConfirm({
      title: "Add Payment?",
  ...
  const finalPayoutsLog = isAdmin 
    ? withdrawals
        .filter(w => payoutHistFilter === 'ALL' ? true : w.status === payoutHistFilter)
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    : userWithdrawals
        .filter(w => w.status === WithdrawalStatus.APPROVED || w.status === WithdrawalStatus.REJECTED)
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  if (view === 'auth') {
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-5 overflow-hidden relative">
        {isSyncing && <LoadingOverlay message="Syncing Profile..." />}
  ...
  if (view === 'onboarding') {
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-5 overflow-y-auto relative py-12">
        {isSyncing && <LoadingOverlay message="Saving Details..." />}
        <Card className="max-w-md w-full p-8 lg:p-10 border-indigo-500/20 shadow-2xl">
          <Logo className="mb-6 justify-center" />
          <div className="text-center mb-10">
            <h2 className="text-2xl font-black text-white uppercase tracking-tight">Complete Profile</h2>
            <p className="text-slate-400 text-xs mt-2 uppercase tracking-widest font-bold">Step 1: Employee Registration</p>
          </div>
          ...
          <form onSubmit={handleOnboardingSubmit} className="space-y-6 text-left">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full bg-[#080C17] border border-slate-800 text-white rounded-xl px-4 py-4 outline-none focus:ring-2 focus:ring-indigo-500/50 text-sm font-bold"
              />
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Place / City</label>
              <input 
                required 
                type="text" 
            ...
            <button 
              type="submit" 
              className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-4 rounded-xl shadow-lg transition-all text-xs uppercase tracking-[0.2em] active:scale-95 mt-4"
            >
            ...
            <button 
              ...
              className="w-full bg-transparent text-slate-600 font-bold py-2 text-[10px] uppercase tracking-widest hover:text-slate-400"
            >
              Cancel & Logout
            </button>
          </form>
    ...
    <div className="h-screen w-screen flex flex-col lg:flex-row bg-[#020617] text-slate-100 font-inter overflow-hidden">
        ...
        <nav className="flex-1 px-4 flex flex-col gap-1">
          {isAdmin ? (
            ...
            <>
              ...
              <NavItem icon={<Icons.Users />} label="Staff" active={view === 'contact'} onClick={() => setView('contact')} />
              <NavItem icon={<Icons.Check />} label="History" active={view === 'history'} onClick={() => setView('history')} />
              <NavItem icon={<Icons.Plus />} label="Payouts" active={view === 'wallet'} onClick={() => setView('wallet')} />
              <NavItem icon={<Icons.Wallet />} label="Payout Hist." active={view === 'payout_hist'} onClick={() => setView('payout_hist')} />
            </>
          )}
      ...
      <main className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
        <header className="h-[70px] lg:h-[90px] flex items-center justify-between px-6 lg:px-10 shrink-0 border-b border-white/5 bg-[#020617]/95 backdrop-blur-md z-[50]">
          <div className="flex flex-col">
            {isAdmin && <Logo className="scale-75 origin-left -mb-1 opacity-80" />}
            <h1 className="text-xl lg:text-2xl font-black text-white tracking-tight uppercase">
              {view === 'payout_hist' && isAdmin ? 'Admin Payout History' : 
               view === 'admin_jobs' ? 'Jobs' : 
               view === 'admin_workers' ? 'Staff Directory' : 
               view === 'history' ? 'Enrollment History' : 
               view === 'admin_payouts' ? 'Payout Requests' : 
               isAdmin ? 'ADMIN CONTROL PANEL' : 
               view === 'jobs' ? 'Jobs' : 
               view === 'home' ? 'Dashboard Overview' : 
               view === 'contact' ? 'Staff Support' : 
               view === 'wallet' ? 'Withdraw Payout' : 
               view === 'payout_hist' ? 'Payout Records' : 
               (view as string).toUpperCase()}
            </h1>
          </div>
          <div className="flex items-center gap-6">
            {view === 'admin_jobs' && (
              <button className="px-5 py-2.5 bg-[#4f46e5] hover:bg-[#4338ca] text-white text-[10px] font-black uppercase rounded-xl flex items-center gap-2 shadow-lg transition-all active:scale-95">
        ...
        <div className="p-6 lg:p-10 overflow-y-auto flex-1 scrollbar-hide max-w-4xl mx-auto w-full pb-24 lg:pb-10">
              ...
              <Card className="p-8">
                ...
                <div className="space-y-4">
                  {pendingWithdrawals.length > 0 ? pendingWithdrawals.map(w => {
                    const worker = workers.find(wr => wr.id === w.userId);
                    return (
                      <div key={w.id} className="p-4 bg-slate-900/40 border border-white/5 rounded-2xl flex justify-between items-center">
                 ...
                 {adminJobsDisplay.map(job => (
                   <Card key={job.id} className="p-0 overflow-hidden border-slate-800">
                      <div className="p-6 space-y-5">
                        ...
                        <button 
                          onClick={() => setViewingStaffForJob(job)}
                        ...
                        >
                           <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">VIEW STAFF</span>
               ...
               <div className="bg-[#0b1222] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                 ...
                 <div className="divide-y divide-white/5">
                   {activeWorkers.map(worker => (
                     <div key={worker.id} className="px-6 py-5 flex items-center justify-between group hover:bg-white/[0.02] transition-colors">
                        <div className="flex items-center gap-4">
                        ...
                        <button 
                          ...
                          className="px-5 py-2 bg-transparent hover:bg-white/5 text-slate-400 hover:text-white text-[10px] font-black uppercase rounded-xl border border-white/10 transition-all active:scale-95"
                        >
                           HISTORY
                        </button>
                     </div>
          ...
          {((view as string) === 'history') && (
             <div className="space-y-6 animate-in fade-in duration-500">
               ...
               <div className="bg-[#0b1222] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                  <div className="px-6 py-4 grid grid-cols-3 border-b border-white/5">
                    <span className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em]">WORKER</span>
                    <span className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em]">EVENT</span>
                    <span className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] text-right">DATE</span>
                  </div>
                  <div className="divide-y divide-white/5">
                    {(isAdmin ? enrollments : enrollments.filter(e => e.userId === user?.id)).slice().reverse().map(enroll => {
                      const worker = workers.find(w => w.id === enroll.userId);
                      const job = jobs.find(j => j.id === enroll.jobId);
                      return (
                        <div key={enroll.id} className="px-6 py-5 grid grid-cols-3 items-center group hover:bg-white/[0.02] transition-colors">
                           <div className="flex items-center gap-3">
                              <Avatar name={worker?.name || '?'} size="w-9 h-9" className="text-xs" />
                              <span className="font-bold text-white text-sm truncate">{worker?.name || 'Unknown'}</span>
                           </div>
                           <span className="text-sm font-medium text-slate-400 truncate">{job?.title || 'Unknown Event'}</span>
                           <span className="text-xs font-bold text-slate-500 text-right uppercase tracking-tighter">
                         ...
                         <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            ...
                            <div className="flex items-center gap-8">
                               ...
                               <div className="flex gap-3">
                                  <button onClick={() => handleApproveWithdrawal(w)} className="px-7 py-3 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-black uppercase rounded-xl shadow-lg transition-all active:scale-95">APPROVE</button>
                                  <button onClick={() => handleRejectWithdrawal(w)} className="px-7 py-3 bg-rose-600 hover:bg-rose-500 text-white text-[10px] font-black uppercase rounded-xl shadow-lg transition-all active:scale-95">REJECT</button>
                               </div>
                            </div>
                         </div>
                      </Card>
            ...
            <div className="space-y-6 animate-in fade-in duration-500 w-full">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-2xl font-black text-white uppercase tracking-tight">
                   {isAdmin ? 'All Payout History' : 'Payout Records'}
                </h2>
                
                {isAdmin && (
                  <div className="bg-[#0b1222] border border-white/5 rounded-[12px] p-1 flex items-center shadow-lg">
                    {(['ALL', WithdrawalStatus.APPROVED, WithdrawalStatus.PENDING, WithdrawalStatus.REJECTED] as const).map(tab => (
                      <button 
                        ...
                        className={`px-5 py-2 text-[9px] font-black uppercase tracking-widest rounded-[9px] transition-all ${
                          payoutHistFilter === tab ? 'bg-[#4f46e5] text-white shadow-md' : 'text-slate-500 hover:text-slate-300'
                        }`}
                      >
                        {tab}
                      </button>
              ...
              <div className="bg-[#0b1222] border border-white/5 rounded-2xl overflow-hidden shadow-2xl min-h-[400px] flex flex-col">
                 <div className="px-8 py-5 grid grid-cols-4 border-b border-white/5 text-slate-600">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em]">WORKER</span>
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-center">AMOUNT</span>
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-center">STATUS</span>
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-right">DATE</span>
                 </div>
                   ...
                   {finalPayoutsLog.length > 0 ? (
                     <div className="divide-y divide-white/5">
                        {finalPayoutsLog.map(w => {
                           const worker = workers.find(wr => wr.id === w.userId);
                           return (
                             <div key={w.id} className="px-8 py-5 grid grid-cols-4 items-center group hover:bg-white/[0.02] transition-colors">
                                <div className="flex items-center gap-3 min-w-0">
                                   <Avatar name={worker?.name || '?'} size="w-8 h-8" className="text-[10px] shrink-0" />
                                   <span className="font-bold text-white text-sm truncate">{worker?.name || 'Unknown'}</span>
                                </div>
                                <span className="text-sm font-black text-white text-center tracking-tight">{w.amount} Rs</span>
                                <div className="flex justify-center">
                                   <span className={`px-2.5 py-1 text-[8px] font-black uppercase rounded-full border ${
                                      w.status === WithdrawalStatus.APPROVED ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                                      w.status === WithdrawalStatus.PENDING ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                     ...
                     <div className="h-full flex items-center justify-center py-20">
                        <p className="text-[11px] text-slate-700 font-black uppercase tracking-[0.3em] italic">NO PAYOUT RECORDS FOUND.</p>
                     </div>
                   )}
                 </div>
              </div>
          ...
          {!isAdmin && view === 'home' && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="flex flex-col gap-4">
                ...
                <StatCard label="ENROLLED" value={enrollments.filter(e => e.userId === user?.id).length.toString()} icon={<Icons.Check />} color="text-indigo-400" onClick={() => setView('history')} />
              ...
              <Card className="px-5 py-4 flex items-center justify-between gap-4 border-indigo-500/10 bg-[#0b1222]">
                ...
                <button onClick={() => setView('payout_hist')} className="px-4 py-2 bg-[#6366f1] hover:bg-[#5558e3] text-white text-[9px] font-black uppercase rounded-[10px] transition-all flex items-center gap-1.5 shadow-lg active:scale-95 shrink-0 h-fit">VIEW ALL <Icons.Check className="w-3 h-3" /></button>
              </Card>
              <Card className="p-8 flex flex-col items-center">
                  ...
                  <div className="w-full max-w-[280px] border border-dashed border-slate-800 rounded-[35px] p-8 flex flex-col items-center justify-center gap-6 group">
                    ...
                    <button disabled={isProcessingQR} onClick={() => fileInputRef.current?.click()} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 text-white text-[10px] font-black uppercase rounded-xl shadow-xl transition-all tracking-widest active:scale-95">{isProcessingQR ? "AI VALIDATING..." : user?.qrCode ? "UPDATE QR" : "LINK UPI QR"}</button>
                  </div>
              </Card>
            </div>
          )}
          ...
          {!isAdmin && view === 'jobs' && (
            <div className="space-y-6 animate-in fade-in duration-500"><h2 className="text-2xl font-black text-white tracking-tight uppercase">Event Opportunities</h2><div className="flex flex-col gap-6 pb-10">{openJobs.map(job => {
                  const isEnrolled = enrollments.some(e => e.userId === user?.id && e.jobId === job.id);
                  return (<div key={job.id} className="relative group"><div className="absolute inset-x-0 -top-[2px] h-[3px] bg-indigo-500/50 rounded-full blur-[1px]"></div><Card className="p-0 overflow-hidden border-[#1e293b]"><div className="p-6 space-y-5"><div className="flex items-start justify-between"><div className="min-w-0"><h3 className="text-xl font-black text-white mb-1.5 tracking-tight uppercase truncate">{job.title}</h3><div className="flex items-center gap-2"><span className="text-[#6366f1] text-xs font-bold">{job.location}</span><button onClick={() => openMap(job.location)} className="px-1.5 py-0.5 bg-[#1e293b] hover:bg-slate-700 text-slate-400 text-[9px] font-black uppercase rounded-[3px] transition-colors">MAP</button></div></div><span className="px-3 py-1 bg-[#052e16] text-[#22c55e] text-[10px] font-black uppercase rounded-full border border-[#14532d] shrink-0">{job.status}</span></div><div className="bg-[#101827] rounded-[20px] p-6 space-y-4 border border-white/5"><div className="flex items-center justify-between"><span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em]">DATE</span><span className="text-base font-black text-white tracking-tight">{job.date}</span></div><div className="flex items-center justify-between"><span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em]">PAYMENT</span><span className="text-base font-black text-[#22c55e] tracking-tight">{job.pay} Rs</span></div><div className="flex items-center justify-between"><span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em]">STAFF</span><span className="text-base font-black text-white tracking-tight">{job.enrolledCount} / {job.maxWorkers}</span></div></div><button onClick={() => handleEnrollAttempt(job)} disabled={!isEnrolled && job.enrolledCount >= job.maxWorkers} className={`w-full py-4 rounded-[14px] text-base font-black uppercase tracking-[0.05em] transition-all shadow-xl active:scale-95 ${isEnrolled ? 'bg-rose-600 hover:bg-rose-700 text-white' : job.enrolledCount >= job.maxWorkers ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-[#6366f1] hover:bg-[#5558e3] text-white'}`}>{isEnrolled ? 'CANCEL ENROLLMENT' : job.enrolledCount >= job.maxWorkers ? 'FULLY BOOKED' : 'ENROLL NOW'}</button></div></Card></div>);
                })}</div></div>
          )}
        ...
        <nav className="lg:hidden h-[60px] bg-[#0b101c] border-t border-white/5 flex items-stretch justify-around fixed bottom-0 left-0 right-0 z-[100] shadow-[0_-8px_30px_rgb(0,0,0,0.5)] overflow-x-auto scrollbar-hide">
          {isAdmin ? (
            <>
              ...
              <NavItem isMobile icon={<Icons.Wallet />} label="Records" active={view === 'payout_hist'} onClick={() => setView('payout_hist')} />
            </>
          ) : (
            <>
              ...
              <NavItem isMobile icon={<Icons.Users />} label="Staff" active={view === 'contact'} onClick={() => setView('contact')} />
              <NavItem isMobile icon={<Icons.Check />} label="Hist" active={view === 'history'} onClick={() => setView('history')} />
              <NavItem isMobile icon={<Icons.Plus />} label="Pay" active={view === 'wallet'} onClick={() => setView('wallet')} />
              <NavItem isMobile icon={<Icons.Wallet />} label="Records" active={view === 'payout_hist'} onClick={() => setView('payout_hist')} />
            </>
          )}
           ...
           <div className="space-y-4">
              {enrollments.filter(e => e.jobId === viewingStaffForJob.id).length > 0 ? (
                enrollments.filter(e => e.jobId === viewingStaffForJob.id).map(enroll => {
                  ...
                  return (
                    <div key={enroll.id} className="flex items-center justify-between p-4 bg-[#080c17] border border-white/5 rounded-2xl group hover:bg-[#0c1426] transition-colors">
                       <div className="flex items-center gap-3">
                       ...
                       <button 
                         onClick={() => handleAddWorkerPay(worker, viewingStaffForJob.pay)}
                         className="px-4 py-2 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-500 hover:text-white border border-emerald-500/20 text-[9px] font-black uppercase rounded-lg transition-all"
                       >
                         ADD PAY
                       </button>
                    </div>
      ...
      {showEnrollConfirm && (
        <Modal centered title="Confirm Enrollment" onClose={() => setShowEnrollConfirm(false)}>
           <div className="text-center">
              <p className="text-xs text-slate-400 font-medium mb-8 leading-relaxed px-4">Are you sure you want to enroll in <span className="text-white font-black">{selectedJobForEnroll?.title}</span>? Please ensure you are available on <span className="text-white font-black">{selectedJobForEnroll?.date}</span>.</p>
              <div className="flex gap-3">
                ...
                <button onClick={() => handleEnrollConfirm()} className="flex-1 py-3 bg-indigo-600 text-white font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl active:scale-95">YES, CONFIRM</button>
              </div>
           </div>
        </Modal>
      )}
