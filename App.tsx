...
export default function App() {
  ...
  const syncUserProfile = async (supabaseUserOrEmail: any) => {
    ...
    try {
      ...
      if (!profile) {
        ...
        // Strict await to ensure record exists before setting state
        await db.saveUser(profile);
      }

      // Re-fetch to ensure we have the most accurate record
  ...
  const handleEnrollConfirm = async (jobOverride?: Job) => {
    ...
    setIsSyncing(true);
    try {
      // Final verification check to fix "Profile connection error"
      // This ensures the worker actually exists in the DB workers table
      let profile = await db.getUserByEmail(user.email);
      if (!profile) {
          // If for some reason the profile is missing, try to recreate it now
