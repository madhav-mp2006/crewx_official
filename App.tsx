...
export default function App() {
  ...
  const handleEnrollConfirm = async (jobOverride?: Job) => {
    ...
    try {
      ...
      setSyncMessage(isEnrolled ? "Cancelling Enrollment..." : "Enrolling...");
                      ...
                      return (
                        <div key={enroll.id} className="px-6 py-5 grid grid-cols-3 items-center group hover:bg-white/[0.02] transition-colors">
                           <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <span className="font-bold text-white text-sm truncate block">{worker?.name || 'Unknown'}</span>
                                {isAdmin && <span className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">{worker?.phone || 'No Phone'}</span>}
                              </div>
                           </div>
                           <div className="min-w-0">
                             <span className="text-sm font-medium text-slate-400 truncate block">{job?.title || 'Unknown Event'}</span>
                    ...
                    <div key={job.id} className="relative group">
                      ...
                      <Card className="p-0 overflow-hidden border-[#1e293b]">
                        <div className="p-6 space-y-5">
                          ...
                          {isEnrolled ? (
                            <div className="flex gap-3">
                              <button disabled className="flex-[2] py-4 bg-emerald-600/10 text-emerald-500 text-base font-black uppercase rounded-[14px] border border-emerald-500/20 cursor-default">ENROLLED</button>
                            ...
                            >
                              {job.enrolledCount >= job.maxWorkers ? 'FULLY BOOKED' : 'ENROLL NOW'}
                            </button>
                          )}
                        </div>
                      </Card>
                  ...
                  return (
                    <div key={enroll.id} className="flex flex-col p-4 bg-[#080c17] border border-white/5 rounded-2xl group hover:bg-[#0c1426] transition-colors space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-white/5">
                           <div className="flex flex-col">
                              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">PHONE</span>
                              <span className="text-[10px] font-bold text-slate-300">{worker.phone || 'N/A'}</span>
                           </div>
                           <div className="flex flex-col">
                              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">AGE / EXP</span>
                              <span className="text-[10px] font-bold text-slate-300">{worker.age || '?'}/{worker.experience || '?'}Y</span>
                           </div>
                           <div className="flex flex-col text-right">
                              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">ENROLLED</span>
                              <span className="text-[10px] font-bold text-slate-300">{new Date(enroll.enrolledAt).toLocaleDateString('en-GB')}</span>
                           </div>
                        </div>
                    </div>
                  );
        ...
        >
           <div className="text-center">
              ...
              <p className="text-xs text-slate-400 font-medium mb-8 leading-relaxed px-4">
                {enrollments.some(e => e.userId === user?.id && e.jobId === selectedJobForEnroll?.id) ? (
                  <>Are you sure you want to <span className="text-rose-500 font-black">cancel your enrollment</span> for <span className="text-white font-black">{selectedJobForEnroll?.title}</span>? This will remove your details from the staff list.</>
                ) : (
                  <>Are you sure you want to enroll in <span className="text-white font-black">{selectedJobForEnroll?.title}</span>? Please ensure you are available on <span className="text-white font-black">{selectedJobForEnroll?.date}</span>.</>
                )}
              </p>
              <div className="flex gap-3">
