                 ...
                 {adminJobsDisplay.map(job => (
                   <Card key={job.id} className="p-0 overflow-hidden border-slate-800">
                      <div className="p-6 space-y-5">
                        ...
                        <button 
                          onClick={() => setAdminExpandedStaffJobId(adminExpandedStaffJobId === job.id ? null : job.id)}
                        ...
                        {adminExpandedStaffJobId === job.id && (
                          <div className="bg-[#080c17] rounded-[18px] p-4 border border-white/5 space-y-3 animate-in slide-in-from-top-2 duration-300">
                             ...
                             {enrollments.filter(e => e.jobId === job.id).length > 0 ? (
                               enrollments.filter(e => e.jobId === job.id).map(enroll => {
                                 const worker = workers.find(w => w.id === enroll.userId);
                                 if (!worker) return null;
                                 return (
                                   <div key={enroll.id} className="flex flex-col p-3 bg-white/5 border border-white/5 rounded-xl space-y-2">
                                       <div className="flex items-center justify-between">
                                         <div className="flex items-center gap-2">
                                         ...
                                         <button 
                                           onClick={() => handleAddWorkerPay(worker, job.pay)}
                                           className="px-3 py-1.5 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-500 hover:text-white border border-emerald-500/20 text-[8px] font-black uppercase rounded-lg transition-all"
                                         >
                                           ADD PAY
                                         </button>
                                       </div>
                      ...
                      <Card className="p-0 overflow-hidden border-[#1e293b]">
                        <div className="p-6 space-y-5">
                          ...
                          <button 
                            ...
                            className="w-full bg-[#1e1b4b]/30 rounded-[12px] px-4 py-2.5 flex items-center justify-between border border-indigo-500/10 hover:bg-indigo-500/20 transition-all active:scale-[0.98] mb-4"
                          >
                          ...
                          {isEnrolled ? (
                            <div className="flex gap-3">
                              <button disabled className="flex-[2] py-4 bg-emerald-600 text-white text-base font-black uppercase rounded-[14px] shadow-xl cursor-default">ENROLLED</button>
