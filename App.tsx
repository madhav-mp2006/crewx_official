...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (session?.user?.email) {
        ...
        const profile = await db.getUserByEmail(user.email);
        if (profile) setUser(profile);
      }
    } catch (err) {
  ...
  const handleDirectWorkerLogin = async () => {
    setIsLoading(true);
    try {
      const demoEmail = 'worker_demo@crewx.com';
      let profile = await db.getUserByEmail(demoEmail);
      if (!profile) {
        profile = {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: {
            parts: [
              ...
              { text: "Examine this image. Is it a clear UPI/Payment QR code? If it is a QR code, answer only 'YES'. If it is anything else (person, scenery, blank, document, etc), answer only 'NO'." }
            ]
          }
        });
        ...
        if (result === 'YES') {
          ...
          setQrWarningMessage("Warning: Invalid Image. Please upload a clear image of your payment QR code only. Other images are not allowed.");
        }
  ...
  const handleEnrollAttempt = (job: Job) => {
    ...
    if (isEnrolled) {
      // Direct cancellation for now or could also ask
        ...
        <Card className="max-w-md w-full p-6 lg:p-10 text-center border-slate-800">
          ...
          {loginRole === UserRole.WORKER ? (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 flex flex-col items-center w-full">
              ...
              <p className="text-slate-400 text-[10px] lg:text-sm mb-8 uppercase tracking-widest font-medium text-center">Enter your digital workspace</p>
              
              {!showWorkerEmailLogin ? (
                <>
                  <button onClick={handleDirectWorkerLogin} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-xl shadow-lg transition-all mb-4 uppercase tracking-widest text-xs active:scale-95 flex items-center justify-center gap-3">
                    Direct Dashboard Access <Icons.Check className="w-4 h-4" />
                  </button>
                  <div className="w-full flex items-center gap-3 my-6 opacity-30">
                  ...
                  <button onClick={() => setShowWorkerEmailLogin(true)} className="w-full text-[10px] text-indigo-400 hover:text-indigo-300 font-bold uppercase tracking-[0.2em] transition-colors py-2 border border-indigo-500/20 rounded-xl">
                    Email Login
                  </button>
                </>
              ) : (
                <form onSubmit={handleWorkerEmailLogin} className="w-full space-y-4 text-left">
                  <input type="email" value={workerEmail} onChange={(e) => setWorkerEmail(e.target.value)} required placeholder="Email Address" className="w-full bg-[#080C17] border border-slate-800 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500/50 text-xs" />
                  <input type="password" value={workerPassword} onChange={(e) => setWorkerPassword(e.target.value)} required placeholder="Password" className="w-full bg-[#080C17] border border-slate-800 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500/50 text-xs" />
                  {loginError && <p className="text-rose-500 text-[10px] font-bold text-center">Invalid credentials</p>}
                  <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all text-xs uppercase tracking-widest active:scale-95">SIGN IN</button>
                  <button type="button" onClick={() => setShowWorkerEmailLogin(false)} className="w-full text-[9px] text-slate-500 font-bold uppercase tracking-widest text-center mt-2">Back to Options</button>
                </form>
              )}
            </div>
          ) : (
            ...
            <div className="space-y-8 animate-in fade-in duration-500 max-w-2xl mx-auto pb-10">
              <Card className="p-8 lg:p-10 text-center flex flex-col items-center">
                ...
                <div className="w-full space-y-4">
                  <button 
                    ...
                    className="w-full py-5 bg-[#1e293b] hover:bg-[#2d3a4f] text-white text-xs font-black uppercase rounded-2xl transition-all shadow-xl active:scale-95 tracking-widest"
                  >
                    EMAIL SUPPORT
                  </button>
                  <button 
                    onClick={handleWhatsAppSupport}
                    className="w-full py-5 bg-[#059669] hover:bg-[#047857] text-white text-xs font-black uppercase rounded-2xl transition-all shadow-xl active:scale-95 tracking-widest"
                  >
                    WHATSAPP CHAT
                  </button>
                </div>
              ...
              <Card className="p-8 lg:p-10 text-center flex flex-col items-center">
                ...
                <button 
                  ...
                  className="w-full py-5 bg-[#6366f1] hover:bg-[#5558e3] text-white text-xs font-black uppercase rounded-2xl transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2 tracking-widest"
                >
        ...
        <Modal centered title="Confirm Payout" onClose={() => setShowWithdrawConfirm(false)}>
           <div className="text-center">
              <p className="text-xs text-slate-400 font-medium mb-8 leading-relaxed px-4">
                ...
                This amount will be deducted from your available balance immediately.
              </p>
              <div className="flex gap-3">
