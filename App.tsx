...
const Avatar: React.FC<{ name: string; size?: string; className?: string }> = ({ name, size = 'w-10 h-10', className = '' }) => {
  ...
  return (
    <div className={`${size} rounded-full flex items-center justify-center font-bold border-2 shadow-sm uppercase ${colorClass} ${className}`}>
       {getInitials(name)}
    </div>
  );
};
...
export default function App() {
  ...
  const handleLogin = async (email: string, password?: string) => {
    ...
    try {
      ...
      if (found && found.role === UserRole.WORKER) { 
        ...
        alert('Worker account not found.'); 
      }
    } catch (e) {
      console.error(e);
      alert("Login failed.");
  ...
  const enrollInJob = async (jobId: string) => {
    ...
    try {
      ...
      // Fix: Use camelCase 'enrolledCount' instead of 'enrolled_count'
  ...
  const cancelEnrollment = async (jobId: string) => {
    ...
    try {
      ...
      await fetchData(true); // Rollback optimistic update
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <Card className="max-w-md w-full p-8">
          ...
          <form onSubmit={(e) => { 
            ...
            if (isSignup) { 
              ...
              handleLogin(
                (f.elements.namedItem('email') as HTMLInputElement).value, 
                (f.elements.namedItem('password') as HTMLInputElement | null)?.value
              ); 
            } 
          }}>
              ...
              {isSignup && (
                <>
                  <div>
                    ...
                    <input name="name" type="text" className="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:ring-indigo-500" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Phone Number</label>
                    <input name="phone" type="tel" className="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:ring-indigo-500" required />
                  </div>
                </>
              )}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Email Address</label>
                <input name="email" type="email" className="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:ring-indigo-500" required />
              </div>
              {!isSignup && loginRole === UserRole.ADMIN && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Admin Password</label>
                  <input name="password" type="password" className="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:ring-indigo-500" required />
                </div>
              )}
          ...
          <div className="mt-6 text-center">
            <button onClick={() => { setIsSignup(!isSignup); setLoginRole(UserRole.WORKER); }} className="text-indigo-600 font-medium hover:underline text-sm">
        ...
        <Modal title="Logout Confirmation" onClose={() => setShowLogoutConfirm(false)}>
          <div className="text-center">
            ...
            <div className="flex gap-4">
              <Button variant="secondary" className="flex-1" onClick={() => setShowLogoutConfirm(false)}>Cancel</Button>
        ...
        <Card className="border-indigo-100 bg-indigo-50/30 p-6 animate-in slide-in-from-top duration-300">
          <form className="space-y-4" onSubmit={async (e) => { 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full"><label className="block text-sm font-semibold mb-1">Event Title</label><input name="title" defaultValue={editingJob?.title} required className="w-full px-3 py-2 rounded border focus:ring-2 focus:ring-indigo-500" /></div>
...
const ContactUsView: React.FC<{ user: User }> = ({ user }) => {
  ...
  const groupLink = "https://chat.whatsapp.com/BCAQOkUYBLL0dTNz5CbvUH?mode=gi_t";
