...
export default function App() {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session?.user) {
        ...
        setUser(profile);
        setView(profile.role === UserRole.ADMIN ? 'admin_dashboard' : 'home');
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isValid) {
        const profile = await db.getUserByEmail(emailToVerify);
        const adminUser: User = profile 
          ? { ...profile, role: UserRole.ADMIN }
          : { id: 'admin-temp', name: 'Admin', email: emailToVerify, role: UserRole.ADMIN, balance: 0 };
        setUser(adminUser);
  ...
  // Admin Job Sorting: Open/Closed first, Completed at bottom
        ...
        <Card className="max-w-md w-full p-6 lg:p-10 text-center border-slate-800">
          ...
          {loginRole === UserRole.WORKER ? (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 flex flex-col items-center w-full">
              ...
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-4">Fast & Secure Access</p>
        ...
        <header className="h-[70px] lg:h-[90px] flex items-center justify-between px-6 lg:px-10 shrink-0 border-b border-white/5 bg-[#020617]/95 backdrop-blur-md z-[50]">
          <div className="flex flex-col">
            ...
            <h1 className="text-xl lg:text-2xl font-black text-white tracking-tight uppercase">
              {view === 'payout_hist' && isAdmin ? 'Admin Payout History' : view === 'admin_jobs' ? 'Jobs' : view === 'admin_workers' ? 'Staff Directory' : view === 'history' ? (isAdmin ? 'Enrollment History' : 'Enrollment History') : view === 'admin_payouts' ? 'Payout Requests' : isAdmin ? 'ADMIN CONTROL PANEL' : view === 'jobs' ? 'Jobs' : view === 'home' ? 'Dashboard Overview' : view === 'history' ? 'Event History' : view === 'contact' ? 'Staff Support' : view === 'wallet' ? 'Withdraw Payout' : view === 'payout_hist' ? 'Payout Records' : view.toUpperCase()}
            </h1>
          </div>
      ...
      {showEnrollConfirm && (
        <Modal centered title="Confirm Enrollment" onClose={() => setShowEnrollConfirm(false)}>
           <div className="text-center">
              ...
              <div className="flex gap-3"><button onClick={() => setShowEnrollConfirm(false)} className="flex-1 py-3 bg-slate-800 text-slate-400 font-bold rounded-xl text-[10px] uppercase tracking-widest active:scale-95">CANCEL</button><button onClick={() => handleEnrollConfirm()} className="flex-1 py-3 bg-indigo-600 text-white font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl active:scale-95">CONFIRM</button></div>
           </div>
        </Modal>
      )}
